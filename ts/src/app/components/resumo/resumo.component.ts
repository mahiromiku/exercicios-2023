...
export class ResumoComponent implements OnInit {
  ...
  textoLimitadoResumo: string = ''

  limitadorDeCaracteresResumo: number = 765
  ...
  trocarCondicaoDoResumo(){
    this.isActiveResume = !this.isActiveResume
  }

  limitarConteudoDoResumo(){
    const conteudoLimitado = this.textoCompletoResumo.substring(0, this.limitadorDeCaracteresResumo) + '...'
    this.textoLimitadoResumo = conteudoLimitado;
    return this.textoLimitadoResumo
  }

  exibirTodoConteudoDoResumo(){
    return this.textoCompletoResumo;
  }
}
